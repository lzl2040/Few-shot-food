...
class AttentionNeuralProcessHead(BaseFewShotHead):
    def __init__(self, self_input_dim, cross_input_dim, q_dim, v_dim, spatial_size, class_num,
                 head = 8, dropout = 0, use_latent_path = False):
        super().__init__()
        ...
        # anp
        self.deterministic_self_att = SelfAttention(input_dim = self_input_dim, k_dim = q_dim, v_dim = v_dim, head = head)
        self.deterministic_cross_att = CrosssAttention(k_input_dim=cross_input_dim, v_input_dim = v_dim,
                                                       k_dim=q_dim, v_dim=v_dim, head=head)
        if self.latent_path:
    ...
    def forward_train(self, support_feats, support_labels,
        ...
        losses = {}
        loss = self.losses(probs,query_label_one_hot.float())
        losses['loss'] = loss
        return losses
    ...
    def forward_query(self, x, **kwargs):
        Bq, C, H, W = x.shape
        query_feats = x.view(Bq, C, -1).transpose(1, 2)
    ...
    def before_forward_query(self):
        ...
        self.support_feats = self.support_feats.view(Bs, C, -1).transpose(1, 2)
