...
"""
File base_metric.py
Created on 2023/7/7 21:30
Copyright (c) 2023/7/7
@author: 
"""
from model.base import BaseFewShotClassifier
...
class BaseMetricClassifier(BaseFewShotClassifier):
    ...
    def forward_train(self, support_data: Dict, query_data: Dict,
        ...
        if support_data.get('feats', None):
            ...
            support_img = support_data['img'].cuda()
            support_feats = self.extract_feat(support_img)

        if query_data.get('feats', None):
            query_feats = query_data['feats']
        else:
            query_img = query_data['img'].cuda()
            query_feats = self.extract_feat(query_img)

        losses = dict()
        loss = self.head.forward_train(support_feats, support_data['gt_label'].cuda(),
                                       query_feats, query_data['gt_label'].cuda())
        losses.update(loss)
